...
class FluxPipeline:
    ...
    def generate(
        ...
        if not silent:
            logger.info(f"   ⚙️  Preprocessing completed in {prep_time:.2f}s")
        ...
        for step_idx, (t_curr, t_prev) in enumerate(tqdm(
            ...
            if not silent and step_idx == 0:  # Only log first step to avoid spam
                logger.info(f"   🚀 First inference step: {step_time:.3f}s")

        denoise_time = time.time() - denoise_start_time
        inference_speed = len(timesteps) / denoise_time if denoise_time > 0 else 0

        if not silent:
            logger.info(f"   🔄 Denoising completed in {denoise_time:.2f}s ({inference_speed:.2f} it/s)")
        ...
        if not silent:
            logger.info(f"   🎨 VAE decode completed in {decode_time:.2f}s")

        # Timing: encoding to bytes
        encode_start_time = time.time()
        
        result = self.into_bytes(img, jpeg_quality=jpeg_quality)
        
        encode_time = time.time() - encode_start_time
        total_time = time.time() - total_start_time

        if not silent:
            logger.info(f"   📸 Image encoding completed in {encode_time:.3f}s")
            logger.info(f"✅ Generation completed!")
            logger.info(f"   ⏱️  Total time: {total_time:.2f}s")
            logger.info(f"   📊 Time breakdown:")
            logger.info(f"      • Preprocessing: {prep_time:.2f}s ({prep_time/total_time*100:.1f}%)")
            logger.info(f"      • Denoising: {denoise_time:.2f}s ({denoise_time/total_time*100:.1f}%)")
            logger.info(f"      • VAE decode: {decode_time:.2f}s ({decode_time/total_time*100:.1f}%)")
            logger.info(f"      • Encoding: {encode_time:.3f}s ({encode_time/total_time*100:.1f}%)")
            logger.info(f"   🚀 Average inference speed: {inference_speed:.2f} it/s")
